...
export default async function ProjectPage({ params, searchParams }: Props) {
  ...
  return <div className="shell"><header className="topbar"><Link className="brand" href="/projects">{copy.brand}</Link><Link href="/projects">All projects</Link></header>
    <main className="main portal"><div className="eyebrow">{project.location}</div><h1>{project.name}</h1>
      ...
      <form className="filters" method="get"><input type="hidden" name="segment" value={segment.slug} />
        ...
        <button type="submit">Apply filters</button><Link href={`/projects/${slug}?segment=${segment.slug}`}>Clear filters</Link>
      </form>
      ...
      <div className="table-scroll"><table className="document-table"><thead><tr><th>Title</th><th>Number</th><th>Type</th><th>Revision</th><th>Status</th><th>Date</th><th>Size</th><th></th></tr></thead><tbody>
        {documents.map((doc) => <tr key={doc.id}><td>{doc.title}</td><td>{doc.doc_number}</td><td>{doc.doc_type}</td><td>{doc.revision}</td><td>{doc.status.replaceAll("_", " ")}</td><td>{doc.issue_date ?? "-"}</td><td>{(doc.file_size / 1048576).toFixed(1)} MB</td><td><PdfViewerLauncher documentId={doc.id} /></td></tr>)}
      </tbody></table></div>
